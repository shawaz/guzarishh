const { Client, Databases } = require('node-appwrite');

const client = new Client()
  .setEndpoint('http://31.97.229.201/v1')
  .setProject('68b5d113000306db095e')
  .setKey('standard_');

const databases = new Databases(client);
const DATABASE_ID = 'guzarishh-db';

async function fixCartSchema() {
  try {
    console.log('🔧 Fixing cart collection schema...');

    // Add missing addedAt attribute to cart collection
    try {
      await databases.createDatetimeAttribute(DATABASE_ID, 'cart', 'addedAt', true);
      console.log('✅ Added addedAt attribute to cart collection');
      
      // Wait for attribute to be available
      await new Promise(resolve => setTimeout(resolve, 2000));
    } catch (error) {
      if (error.code === 409) {
        console.log('ℹ️  addedAt attribute already exists');
      } else {
        console.error('❌ Error adding addedAt attribute:', error.message);
      }
    }

    console.log('🎉 Cart schema fixed!');
    
  } catch (error) {
    console.error('❌ Failed to fix cart schema:', error);
  }
}

fixCartSchema();
